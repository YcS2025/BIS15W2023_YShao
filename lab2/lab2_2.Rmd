...
```{r}
global <- rowSums(all_harry_potter_matrix_1)
global
```
...
```{r}
USearning <-colSums(harry_potter_matrix)
USearning
```

2. Add this information to the data matrix. Hint: you are adding a row, not a column.  
```{r}
all_harry_potter_matrix_1 <- rbind(harry_potter_matrix, USearning)
all_harry_potter_matrix_1
```
