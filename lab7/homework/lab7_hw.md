...
For birds and reptiles, I think that the relevant data may not be available yet, so NA is temporarily used instead. For mammals, since most mammals do not lay eggs, such NA should be meaningful.
...
## 2     1  2429
```
It is not difficult to see that for each classify species, the sum of the number of 1 plus the number of NA is 6776. Therefore, NA here means that this creature does not belong to the current classify species.
